...
def Div_Diff(x_list, func):
    ...
    for j in range(len(x_list)):
        ...
        divided_differences += func(x_list[j])/p
    return divided_differences

def Newton(x_list , x_for_interpolation, func=lambda x: 1/(1+x**2)):
    y_interpolated_list = []
    for x in x_for_interpolation:
        ...
        for n in range(len(x_list)):
            ...
            y_interpolated += diviaded_values*Div_Diff(x_list[0:n+1],func=func)
